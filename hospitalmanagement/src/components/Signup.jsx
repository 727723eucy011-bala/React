...
//         if (!validateUsername(username)) {
//             setError("Username must be between 2 and 30 characters and contain only letters and numbers.");
//             return;
//         }
...
const Signup = () => {
    ...
    const validatePassword = (password) => {
        const passwordRegex = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]).{8,}$/; // At least one letter, one number, one special character and at least 8 characters long
        return passwordRegex.test(password);
    };
                    ...
                    <Card variant="outlined">
                        ...
                        <Box component="form" onSubmit={handler} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            ...
                            <TextField
                                ...
                                helperText={!validatePassword(password) && password.length > 0 ? "Invalid password." : ""}
                            />
                            <Typography>Password should contian atleast one character, number , symbol and length should greate than or equal to 8</Typography>
